...
def post_detail(request, id=None):
	...
	extra_tag_list=instance.extra_tag_set.all()
	...
	print("extra tag list")

	for extra_tag_obj in extra_tag_list:
		temp2={}
		count2=i%4
		temp2["color"]=color[count]
		temp2["tagname"]=extra_tag_obj.name
		tag_list2.append(temp2)
		i=i+1
		print(extra_tag_obj)
	print(extra_tag_list)
...
def photolink(request):
	try:
		...
		if search_query:
			...
			extra_tag=search_in_waston(search_query.split(" "),search_model.extra_tag)
			if extra_tag:
				context["extra_tag"]=(extra_tag[0].first()).name
				print(extra_tag)
...
def search(request):
	...
	if 'hobbytag' in search_text.keys():
		...
		if 'extratag' in search_text.keys():
			place_list=search_model.extra_tag.objects.get(name=search_text['extratag']).places.all()
			print(place_list)
...
def update_place(request,user_id=None):
	...
	try :
		...
		peopleid.save()
		try:
			user_record_obj=User_record.objects.get(user_id=peopleid.peopleid,place_id=instance.place_id)
			print("INSERTONE")
			data=user_record_obj.data
			try:
				if data[textid]:
					lastpoint=(data[textid])["point"]
			except Exception as e:
				lastpoint='0'

			temp={}
			temp["text"]=text
			temp["point"]=int(adminpoint)+int(lastpoint)
			data[textid]=temp
			user_record_obj.data=data
			...
			temp={}
			temp["text"]=text
			temp["point"]=int(adminpoint)
			data[textid]=temp
			user_record_obj=User_record(user_id=peopleid,place_id=instance,data=data)
			user_record_obj.save()
		...
		if request.method=="POST":
			if request.session.get("sessionid",None):
				...
				try:
					...
					photo_pic_obj=user_profile_pic.get(user_id=People(sessionid))
					photo_pic_obj.photo=img
					photo_pic_obj.save()
				except Exception as e:
					photo_pic_obj=user_profile_pic(photo=img,user_id=People(sessionid))
					photo_pic_obj.save()
				request.session["sessionpic"]="/media/"+str(img)
				return HttpResponse()
...
def contributors(request):
	...
	place_id=json.loads(request.body.decode('utf-8'))["place_id"] 
	user_record_obj_list=User_record.objects.filter(place_id=place_id)
	contributor_list=[]
	#contributor_list.append(temp)
	for obj in user_record_obj_list:
		temp={}
		temp["name"]=obj.user_id.username
		temp["pic"]=obj.user_id.photo
		temp["list"]=list(obj.data.keys())
		contributor_list.append(temp)
	print(contributor_list)
	#print(People.objects.get(peopleid=obj.user_id).username)
	...
	if request.method == "POST":
		...
		try:
			...
			for followers in follow_obj:
				...
				credibilitypoints = people_obj.point
		...
		try:
			...
			for articles in article_obj:
				...
				credibilitypoints=articleuserid.point
		...
		try:
			...
			for photoposts in photopost_obj:
				...
				credibilitypoints = int(photopostuserid.point)
				reportstatus = photoposts.report
				...
				print("helloxyzxxxxxxxxxxxxxccccccccccccccccccccccccccccccccccccccccccccccccccxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")	
			
				print(count_photopost_like_points)
				print(count_photopost_superlike_points)
				print(count_photo_report)
			countsmlike=0
			countmdlike=0
			countlglike=0
			countsmsuperlike=0
		...
		try:
			...
			for trips in trip_obj:
				...
				credibilitypoints = tripuserid.point
		...
		try:
			...
			for tips in tip_obj:
				...
				credibilitypoints = int(tipuserid.point)
			...
			print(count_tip_report)
			print(count_tip_helpful_points)
			countsmlike=0
			countmdlike=0
			countlglike=0
			countsmsuperlike=0
		...
		try:
			...
			for questions in question_obj:
				...
				credibilitypoints = int(questionuserid.point)
			...
			print(count_question_report )
			print(count_question_helpful_points )
			countsmlike=0
			countmdlike=0
			countlglike=0
			countsmsuperlike=0
		...
		try:
			...
			for answers in answer_obj:
				...
				credibilitypoints = answeruserid.point
		...
		try:
			...
			for reviews in review_obj:
				...
				credibilitypoints = reviewuserid.point
		...
		print("sccr")
		print(count_tip_helpful_points * helpfultippoint + count_question_helpful_points * questionhelpfulpoint + count_answer_helpful_points * answerhelpfulpoint + count_review_points * reviewhelpfulpoint)
